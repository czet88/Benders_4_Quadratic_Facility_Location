...
void Benders_root_node(void)
{
	...
	if (Capacitated_instances == 1){
		...
		free(rhs);
													...
													//Adding a combinatorial cut to see i5f it does the trick
	/*numrows =countsols;
	numnz = countsols*(NN+1);
	d_vector(&rhs,numrows,"open_cplex:2");
	c_vector(&sense,numrows,"open_cplex:3");
	i_vector(&matbeg,numrows,"open_cplex:4");
	i_vector(&matind,numnz,"open_cplex:6");
    ...
    index = 0;
    index1 = 0;
	//printf("We have %d previous solutions\n",countsols);
	for(k=0;k<countsols;k++){
		sense[index1]='G';
		rhs[index1]= prevsols[k].preeta*(1-prevsols[k].num_comb);
		//printf("num_comb=%d\n",prevsols[k].num_comb);
		matbeg[index1++] = index;
		matind[index] = pos_eta;
		matval[index++] = 1;
		for(i=0;i<NN;i++){  
			matind[index] = pos_z[i][prevsols[k].indhub[i]];
			matval[index] = -prevsols[k].preeta;
			index++;
		}
	}
	status = CPXaddrows (env, lp, 0, index1, index, rhs, sense, matbeg, matind, matval, NULL, NULL);
	...
	free(sense);
	free(rhs);*/
	                                                // Add initial set of optimality cuts associated with heuristic solution
	/*for (i = 0; i < NN; i++) {
		for (k = 0; k < NN; k++) {
			if (initial_x[pos_z[i][k]] == 1)
				printf("z[%d][%d]=1 \n", i + 1, k + 1);
		}
	}*/
	/****Here we've commented to see what happens if we don't add initial cuts****/
	//printf("Started solving original cuts\n");
	//index = 0;
	//index1 = 0;
	//start_SP = clock();
	//if (MG == 0 || MG==1)
	//	Update_Core_Point(initial_x);
	//for (i = 0; i<NN; i++) {                        //Solve (i,j) primal/dual subproblems
	//	for (j = 0; j<NN; j++) {
	//		//if(MG==1) Update_CP_MW(initial_x,i,j);
	//		status = NetFlow_TP(initial_x, i, j);
	//	}
	//}
   ...
   do{
			   ...
			   for (k = 0; k < NN; k++) {
				   if (x[pos_z[i][k]] > 0.00001)
					   printf("z[%d][%d]=%.2f \n", i + 1, k + 1, x[pos_z[i][k]]);
										 ...
										 //Elimination tests with reduced cost coefficients
		   flag_fixed = 0;
		   if(vers!=5){
		   ...
		   if ((iter>10 && iter % 3 == 0 && vers != 4 && hybrid == 0) || (hybrid == 1 && iter>0 && iter % 3 == 0)){  //Special procedure repeated at first iteration and at every fourth
			   flag_fixed = 0;
			   count_c = 0;
			   for (k = 0; k < NN; k++){
	   ...
	   if(flag_fixed==1){
		   ...
		   CPXsolution(env, lp, &status, &value, x, NULL, NULL, dj);
	   	   	   										  ...
	   	   	   										  //Solving the subproblem to find Benders cuts
	   start_SP = clock();
	   iter++;
	   flag_added = 0;
	   index = 0;
	 ...
	 //  printf("\n");
	   ...
	   //for (k = 0; k < NN; k++) {
		  // if (fixed_zero[k] == 0 && x[pos_z[k][k]] > 0.005 && b[k] - O[k] >= 0 && AggregatedDemand > b[k]){
			 //  for (i = 0; i < NN; i++) {									//Sort the N nodes in non-decreasing order
				//   ord_nodes[i].i = i;
				//   ord_nodes[i].coeff = ((x[pos_z[k][k]] - x[pos_z[i][k]]) / O[i]);
				//   ord_nodes[i].removed = 0;
			 //  }
			 //  qsort((ZVAL *)ord_nodes, NN, sizeof(ord_nodes[0]), Comparevalue_ord);

			 //  aa = (b[k] - O[k]);										//Construct cover
			 //  do{
				//   sum_O = 0;
				//   max_O = 0;
				//   is_cover = 0;
				...
				//	   cover.z[i] = 0;
				//   for (i = 0; i < NN; i++) {
				//	   if (O[ord_nodes[i].i] <= (aa - 0.0001) && ord_nodes[i].i != k){
				//		   cand_cover[count_cover++] = ord_nodes[i].i;
				//		   cover.z[ord_nodes[i].i] = 1;
				//		   sum_O += O[ord_nodes[i].i];
				//		   sum_x += x[pos_z[ord_nodes[i].i][k]];
				//		   if (O[ord_nodes[i].i] > max_O)
				//			   max_O = O[ord_nodes[i].i];
				//		   if (sum_O >(b[k] - O[k])){
				//			   is_cover = 1;
				//			   break;
				//		   }
				//	   }
				//   }
				//   if (is_cover == 1){								//Constrcut extended cover
				//	   for (i = 0; i < NN; i++) {
				//		   if (cover.z[i] = 0 && O[i] >= max_O && i != k){
				//			   cover.z[i] = 1;
				//			   cand_cover[count_cover++] = i;
				//			   sum_x += x[pos_z[i][k]];
				//		   }
				//	   }
				//	   if (sum_x - (count_cover - 1)*x[pos_z[k][k]] > tol_sep_cover){	//Add if violated
				//		   index1 = 0;
				//		   index = 0;
				//		   sense[index1] = 'L';
				//		   rhs[index1] = 0;
				//		   matbeg[index1++] = index;
				//		   for (i = 0; i < count_cover; i++){
				//			   matind[index] = pos_z[cand_cover[i]][k];
				//			   matval[index++] = 1;
				//		   }
				//		   matind[index] = pos_z[k][k];
				//		   matval[index++] = -(count_cover - 1);
			 ...
			 //  eval_cut = Solve_Exact_Separation(k, x);
			 //  if (eval_cut > 0.000001){	//Add if violated
				//   index1 = 0;
				//   index = 0;
				//   sense[index1] = 'L';
				//   rhs[index1] = 0;
				...
				//	   if (i != k){
				//		   matind[index] = pos_z[i][k];
				//		   matval[index++] = coeff_ES[i];
				//	   }
				//	   else{
				//		   matind[index] = pos_z[k][k];
				//		   matval[index++] = -(coeff_ES[k]);
				//	   }
				//   }
	 ...
	 //  if (flag_added != 0) {
      // if (flag_added != 0 && ABS(value - old_objval) > epsilon_LP) {
...
int Comparevalue_ord(const void *a, const void *b)
{
	if (((NORD *)a)->coeff>((NORD *)b)->coeff)
		return 1;
	if (((NORD *)a)->coeff<((NORD *)b)->coeff)
		return -1;
	return 0;
